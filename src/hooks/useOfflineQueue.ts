...
export const useOfflineQueue = () => {
    ...
    const addOfflineOrder = useCallback(async (order: Order): Promise<void> => {
        try {
    ...
    const processOfflineOrders = useCallback(async (): Promise<{
        ...
        if (!networkStatus.isConnected || state.isProcessing) {
            console.log('⏸️ Cannot process orders: network disconnected or already processing');
    ...
    const removeOfflineOrder = useCallback(async (orderId: string): Promise<void> => {
        try {
    ...
    useEffect(() => {
        if (networkStatus.isConnected && state.offlineOrders.length > 0 && !state.isProcessing) {
    ...
    }, [networkStatus.isConnected, state.offlineOrders.length, state.isProcessing, processOfflineOrders]);
