...
export class OfflineQueueService {
  ...
  static async processOfflineOrders(): Promise<{
    ...
    try {
      ...
      console.log(
        `✅ Offline orders processed: ${successful} successful, ${failed} failed`,
      );
    } catch (error) {
  ...
  private static async processOrder(order: Order): Promise<boolean> {
    try {
      ...
      if (response.success) {
        ...
        console.log(
          `❌ Order ${order.id} submission failed: ${response.message}`,
        );
  ...
  private static async scheduleRetry(
    ...
    if (retryCount > RETRY_CONFIG.MAX_RETRIES) {
      console.log(
        `💀 Order ${order.id} exceeded max retries (${RETRY_CONFIG.MAX_RETRIES}), marking as failed`,
      );
    ...
    console.log(
      `⏰ Scheduling retry ${retryCount}/${RETRY_CONFIG.MAX_RETRIES} for order ${order.id} in ${delay}ms`,
    );
  ...
  static async retryFailedOrders(): Promise<void> {
    try {
      const offlineOrders = await StorageService.getOfflineOrders();
      const failedOrders = offlineOrders.filter(
  ...
  static async getQueueStats(): Promise<{
    ...
    try {
      ...
      const pending = offlineOrders.filter(
        (order) => order.status === "pending",
      ).length;
      const failed = offlineOrders.filter(
        (order) => order.status === "failed",
      ).length;
